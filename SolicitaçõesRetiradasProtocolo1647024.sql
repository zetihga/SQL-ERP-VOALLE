...
WHERE it.id in (219,1051,1052,1053,1054,1055,
 ...
 1435,1436,1437,1438,1439,1440,1441,1442,1443,1444,1445,1447,1493,1600,1312,1437,1438)
and t.code = 4.8
 -- and a.conclusion_date > '2021-04-01'
 -- and a.beginning_date > '2020-01-01'

select * from incident_types it
where it.title like '%Retirada%'

select * from bank_accounts ba
join financial_receivable_titles frt on ba.id = frt.bank_account_id
join contracts c on frt.contract_id = c.id
join people p on c.client_id = p.id
where ba.description like '%03384-7%'
and frt.balance > 0
and frt.deleted = 0
